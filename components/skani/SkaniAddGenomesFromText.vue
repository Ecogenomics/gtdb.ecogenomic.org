...
import {SkaniValidateGenomesRequest} from "~/assets/api/skani";
...
export default Vue.extend({
  ...
  methods: {
    ...
    emitGenomeIds() {
      const request: SkaniValidateGenomesRequest = {
        genomes: this.genomeAccessions
      }
      this.isApiRunning = true;
      this.$api.skani.validateGenomes(request).then(res => {
        const rows: SkaniTableRow[] = [];
